...
const moveBall = () => {
    ...
    // hit paddle
    if (ball.y + ball.size >= paddle.y && ball.x + ball.size >= paddle.x && ball.x - ball.size <= paddle.x + paddle.w) {
        ball.dy *= -1
        return
    }
    // hit bricks
...
const drawScore = () => {
    ctx.font = '20px Arial'
    ctx.fillText(`${score > bestScore ? 'Best Score:' : 'Score:'}${score}`, canvas.width - 120, 30)
}
// create brick props
const brickInfo = {
    w: 70,
